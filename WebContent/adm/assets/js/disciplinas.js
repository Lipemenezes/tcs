...
//clica do deletar que chamao o ajax passando o ID
   $("#btn_deletar").click(function () {
    ...
    //cadastro 
   $("#frm_disciplina").submit(function () {
       var json = '{"id": "'+$("#id").val()+'","curso": "'+$("#curso").val()+'", "professor": "'+$("#professor").val()+'", "disciplina": "'+$("#disciplina").val()+'" }';
       if( $("#curso").val() === '' || $("#professor").val() === '' || $("#disciplina").val() === ''){
...
function getJSON( act ){
  ...
  if( url.indexOf("del") > -1 || url.indexOf("up") > -1 ){
      $.getJSON("http://localhost/tcs/adm/assets/js/disciplina_individual.json", function(data) {
  ...
  else if( url.indexOf("cad") > -1 == false)
  {
         $.getJSON("http://localhost/tcs/adm/assets/js/disciplina.json", function(data) {
            ...
            for (j = 0; j < d.length; j++) {
                  ...
                  saida +='<td>';
                  saida +='<a href="crud_disciplina.html?id='+d[j].id+'&act=up" class="btn-floating green"><i class="material-icons">edit</i></a>';
                  saida +='<a  href="crud_disciplina.html?id='+d[j].id+'&act=del"  class="btn-floating red" "><i class="material-icons">delete</i></a>';
                  saida +='</td>';
                  saida +='</tr>';
              }
...
function getCOMBO( idCurso, idProfessor){
 ...
 if( idCurso === '' )  
     $('select[name="curso"]').append("<option  value=\"\">Selecione um curso</option>"); 
  ...
  for (i = 0; i < c.length; i++) {
      ...
      if( idCurso.valueOf() !== '' ){
          if( idCurso.valueOf() === c[i].id ){
             $('select[name="curso"]').append("<option selected value=\"" + c[i].id + "\">" + c[i].nome + "</option>");
          }else{
                $('select[name="curso"]').append("<option  value=\"" + c[i].id + "\">" + c[i].nome + "</option>");
          }
      }else{
            $('select[name="curso"]').append("<option  value=\"" + c[i].id + "\">" + c[i].nome + "</option>");
      }
