...
function getJSON(  ){
  ...
  if( url.indexOf("del") > -1 || url.indexOf("up") > -1 ){
      $.getJSON("http://localhost/tcs/adm/assets/js/usuario_individual.json", function(data) {
          $("#nome").val(data.nome);
  ...
  else if( url.indexOf("cad") > -1 == false)
  {
         ...
         $.getJSON("http://localhost/tcs/adm/assets/js/usuario.json", function(data) {
        var d = data.usuario;
        var saida = "";

       for (j = 0; j < d.length; j++) {
             saida += ' <tr> ';
             saida +='<td>'+d[j].nome+' ' +d[j].sobrenome+ '</td>'
             saida +='<td>'+d[j].telefone+'</td>'
             saida +='<td>'+d[j].email+'</td>'
             saida +='<td>'+d[j].tipo+'</td>'
             saida +='<td>';
             saida +='<a href="crud_usuario.html?id='+d[j].id+'&act=up" class="btn-floating green"><i class="material-icons">edit</i></a>';
             saida +='<a  href="crud_usuario.html?id='+d[j].id+'&act=del"  class="btn-floating red" "><i class="material-icons">delete</i></a>';
             saida +='</td>';
             saida +='</tr>';
         }
